...
private:
  ...
  ShenandoahAllocRequest(size_t _min_size, size_t _requested_size, Type _alloc_type, ShenandoahRegionAffiliation affiliation, ShenandoahRegionAccessRate access_rate) :
          _min_size(_min_size), _requested_size(_requested_size),
          _actual_size(0), _alloc_type(_alloc_type), _affiliation(affiliation), _access_rate(access_rate)
#ifdef ASSERT
...
public:
  static inline ShenandoahAllocRequest for_tlab(size_t min_size, size_t requested_size) {
    return ShenandoahAllocRequest(min_size, requested_size, _alloc_tlab, ShenandoahRegionAffiliation::YOUNG_GENERATION, ShenandoahRegionAccessRate::NEUTRAL);
  }
  ...
  static inline ShenandoahAllocRequest for_gclab(size_t min_size, size_t requested_size, ShenandoahRegionAccessRate access_rate) {
    return ShenandoahAllocRequest(min_size, requested_size, _alloc_gclab, ShenandoahRegionAffiliation::YOUNG_GENERATION, access_rate);
  }
  ...
  static inline ShenandoahAllocRequest for_shared(size_t requested_size) {
    return ShenandoahAllocRequest(0, requested_size, _alloc_shared, ShenandoahRegionAffiliation::YOUNG_GENERATION, ShenandoahRegionAccessRate::NEUTRAL);
  }
