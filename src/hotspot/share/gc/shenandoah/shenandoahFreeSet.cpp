...
HeapWord* ShenandoahFreeSet::allocate_with_affiliation_and_access_rate(ShenandoahRegionAffiliation affiliation, ShenandoahRegionAccessRate access_rate, ShenandoahAllocRequest& req, bool& in_new_region) {
  for (size_t c = _collector_rightmost + 1; c > _collector_leftmost; c--) {
    // size_t is unsigned, need to dodge underflow when _leftmost = 0
    size_t idx = c - 1;
    if (is_collector_free(idx)) {
...
HeapWord* ShenandoahFreeSet::allocate_single(ShenandoahAllocRequest& req, bool& in_new_region) {
  ...
  switch (req.type()) {
    ...
    case ShenandoahAllocRequest::_alloc_shared_gc: {
      // First try to fit into a region that is already in use in the same generation and access_rate.
      HeapWord* result = allocate_with_affiliation_and_access_rate(req.affiliation(), req.access_rate(), req, in_new_region);
      if (result != NULL) {
        return result;
      }
      // Then try a free region that is dedicated to GC allocations.
      result = allocate_with_affiliation_and_access_rate(FREE, NEUTRAL, req, in_new_region);
      if (result != NULL) {
        return result;
      }
