...
inline oop ShenandoahHeap::evacuate_object(oop p, Thread* thread) {
  ...
  if (mode()->is_generational() && ShenandoahHeap::heap()->is_gc_generation_young() &&
    ...
    printf("Is in generational mode");
...
inline oop ShenandoahHeap::try_evacuate_object(oop p, Thread* thread, ShenandoahHeapRegion* from_region, ShenandoahRegionAffiliation target_gen) {
  ...
  ShenandoahRegionAccessRate access_rate = ShenandoahRegionAccessRate::NEUTRAL;
  if (p->access_counter() > hotness_threshold){
    access_rate = ShenandoahRegionAccessRate::HOT;
  }
  else {
    access_rate = ShenandoahRegionAccessRate::COLD;
  }
      ...
      switch (target_gen) {
        ...
        case OLD_GENERATION: {
           if (ShenandoahUsePLAB) {
            ...
            printf("%s | %s | line %d | Copy from old gen\n", __FILE__, __func__, __LINE__);
           }
           break;
        }
        default: {
  ...
  if (result == copy_val) {
    ...
    printf("%s | %s | line %d | result != copy_val\n", __FILE__, __func__, __LINE__);
